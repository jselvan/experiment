...
from typing import Literal, Sequence, TYPE_CHECKING, TypedDict, Optional
...
class RewardAdapter(BaseAdapter):
    DEFAULT_PROGRESS_PARAMS: ProgressParams = {
        'position': (100, 100),
        'size': (200, 30),
        'colour': (0, 255, 0),
        'gap': 10
    ...
    def __init__(self, 
        ...
        children: Sequence[BaseAdapter] = [],
        progress_params: Optional[ProgressParams] | Literal['none'] = None,
        **kwargs):
        ...
        if progress_params is None or progress_params == 'none':
            progress_params = self.DEFAULT_PROGRESS_PARAMS
    ...
    def from_manager(cls, manager: "Manager", duration, n_pulses, interpulse_interval, **kwargs) -> "RewardAdapter":
        ...
        reward_callbacks = manager.good_monkey(duration=None, speed=speed, channels=channels, return_callbacks=True)
    ...
    def render(self, renderer: "Renderer"):
        for child in self.timecounters + list(self.children):
